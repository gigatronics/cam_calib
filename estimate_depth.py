...
def extract_corners(cmd_path,cmd, img, out): #need dsc file in the current working path
    full_cmd = cmd_path+cmd+img+out
...
def find_corners(img):
    ...
    extract_corners(cmd_path,cmd,'img_temp.png',' cam')
...
#    del cornersR[:]
    ...
    path = r'C:\Users\demoPC\py\cam_calib\output_stitched\png_tripod\3m\\'
    path = r'D:\data\frames_out_nov1_axis\out_chrom0_png\\'
    path = r'C:\Users\demoPC\py\cam_calib\output_ids\png_int_ext_model_pertub\\'
...
#
    ...
    parser = OptionParser()
    parser.add_option('-s','--src_path', action="store", type="string", dest = 'src_path')
    parser.add_option('-d','--dst_path', action="store", type="string", dest = 'dst_path')
    ...
    cmd_path = r'C:\Users\demoPC\Desktop\SuoCalibration_v2.6.0a\oCametry\application\checkerDetect\\'
    cmd = r'FindCodedChecker.exe ' 
    
    # create columns and output file.. 
    ...
    for file in sorted(glob.glob(src_path+'*.png')):  # key=os.path.getmtime
#    while True:
        print(file)   
        ...
        extract_corners(cmd_path,cmd,'imgL.png',' camL')
        time.sleep(4)                # takes time to write to file 
        cornersL = read_csv('camL0.txt')
    
        cv2.imwrite('imgR.png',imgR)  
        extract_corners(cmd_path,cmd,'imgR.png',' camR')
        time.sleep(4)                # takes time to write to file 
...
#        cornersR = find_corners_cv(imgR)
 ...
 #        print('right eye: found %g corners' % len(cornersR)) 
    ...
    #            depth_est_error = abs((depth_est-DEPTH_EXP)/DEPTH_EXP*100)
        ...
        # plot
        if not (len(cornersL)==0 and len(cornersR)==0): 
            plt.figure()
...
#            crpD = crop2(disparity, rect)
            ...
            plt.subplot(211), plt.imshow(crpL)
            plt.subplot(212), plt.imshow(crpR)
 #           plt.subplot(313), plt.imshow(crpD)
            #plt.imsave()
            #plt.show() 
            fig = plt.gcf()
            fig.savefig(dst_path+basename+'_plot.png')
